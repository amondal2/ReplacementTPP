...
main_out <- "/scratch/ReplacementTPP/BurkinaFaso/lowEIR"
###############################################################################
### Factorial Sweeps
###############################################################################
  ...
  fun = function(x) {
    ...
    cube <- MGDrivE::cubeHomingDrive(
      ...
      eta = list(
        c('WW', 1.0),
        c('WR', 1.0),
        c('WB', 1.0 - (1.0 - PARS[[x, 'etaB']])),
        c('WH', 1.0 - (1.0 - PARS[[x, 'etaH']])),
        c('HH', 1.0 - 2 * (1 - PARS[[x, 'etaH']])),
        c('HR', 1 - (1 - PARS[[x, 'etaH']])),
        c('HB', 1 - (1 - PARS[[x, 'etaB']]) - (1 - PARS[[x, 'etaB']])),
        c('RR', 1),
        c('RB',  1.0 - (1.0 - PARS[[x, 'etaB']])),
        c('BB', 1.0 - 2 * (1 - PARS[[x, 'etaB']]))
      )
    )
    ...
    ft <- 0.10 # percent of symptomatic cases that are treated
    ...
    theta <- eqm$theta
    theta$b0[grep(pattern = "H",x = names(theta$b0))] <- PARS[[x, 'b']]
    cube <- eqm$cube
    ...
    carry <-
      read.csv2(file = "data/carrying_capacity_bf.csv",
                sep = ",",
