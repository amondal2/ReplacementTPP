...
# Drive constants
###############################################################################
FC_B = 0.117    # sB = 0.117 (95% CrI: 0.095-0.138) = Fitness cost (measured as reduced contribution of alleles to the next generation) of females & males having one copy of the B allele
FC_H = 0.175    # sH = 0.175 (95% CrI: 0.169-0.181) = Fitness cost (measured as reduced contribution of alleles to the next generation) of males having one copy of the H allele
FC_R = 0
CUT_M = 0.980   # c_M = 0.980 (95% CrI: 0.980-0.981) = Proportion of W alleles that are cleaved in the gametes of male HW heterozygotes
CUT_F = 0.999   # c_F =  (95% CrI: 0.994-1.00) = Proportion of W alleles that are cleaved in the gametes of female HW heterozygotes
pRES = 0.079    # pRES = 0.079 (95% CrI: 0.068-0.092) = Proportion of newly-formed resistance alleles that are in-frame, functional (R) vs. out-of-frame or otherwise costly (B)
...
HDR_F = 0.956   # pHDR_F = 0.955 / 0.999 = 0.956 = Proportion of cleaved W alleles that are converted into H alleles in the gametes of female HW heterozygotes
...
# dummy b-matrix - TODO fill with real data
...
# need to pass in mu and EIP parameters to properly set up equilibrium distribution
imperial_params <-
  imperial_model_param_list_create(muF = theta$muF,
                                   nEIP = theta$nEIP,
                                   qEIP = theta$qEIP)
...
EIR <-
  3 # desired annual EIR from which to calculate human state distribution

threshold <- "X10000"

human_eqm <-
  equilibrium_Imperial_decoupled_human(age_vector, ft, EIR, imperial_params)
...
# calculate initial mosquito distribution based on human
# and cube params
...
carry <- read.csv2(file = "data/carrying_capacity_stp.csv",sep = ",",stringsAsFactors = FALSE)
...
make_hazards <- function(spn_P,spn_T,cube,par,log_dd=TRUE,exact=TRUE,tol=1e-12,verbose=TRUE){
  ...
  for(t in 1:n){
    ...
    if(type == "oviposit"){
      ...
      h[[t]] <- MGDrivE2::make_male_mort_haz(t = spn_T$T[[t]],u = u,cube = cube,par = par,exact = exact,tol = tol)
      # INHOMOGENEOUS
    } else if(type %in% c("female_mort","female_unmated_mort")){
      h[[t]] <- MGDrivE2::make_female_mort_haz(t = spn_T$T[[t]],u = u,cube = cube,par = par,exact = exact,tol = tol)
...
tau_humans <- summarize_humans_epiImperial(out = tau_out$state, index=2)
x <- as.data.frame(tau_out$state[,,1])
inc <- dplyr::select(x, dplyr::contains("clin_inc"))
inc_total <- rowSums(inc)
plot(inc_total)
 # plot
